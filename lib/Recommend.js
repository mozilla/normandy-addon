...
class Recommender {
  constructor() {
    ...
    this.urlButton = this.window.document.createElement('image');
    this.urlButton.id = 'recommend-button';
    this.urlButton.setAttribute('class', 'urlbar-icon');
    this.urlButton.setAttribute('src', data.url('recommendation/icon.png'));
    this.urlButton.setAttribute('hidden', 'true');

    this.urlButton.addEventListener('click', this.showPanel.bind(this));
    this.window.document.getElementById('urlbar-icons').appendChild(this.urlButton);
